PlayerEvents.tick(event => {
    ...
    const playerName = event.player.getDisplayName().getString()
    const scores = event.server.getScoreboard().getOrCreatePlayerScore(playerName, scoreboard)
})

ItemEvents.rightClicked(event => {
    try {
        if (event.item.id == "minecraft:stick") {
            event.player.experienceProgress = event.player.experienceProgress + 0.01
            event.player.giveExperiencePoints(0)
